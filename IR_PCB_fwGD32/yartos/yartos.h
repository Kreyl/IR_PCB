...
#define TIME_MAX_SYSTIME    ((systime_t)-1) // Maximum value of system time before it wraps.

#define TIME_S2I(secs)   ((systime_t)(  (uint64_t)(secs)  * (uint64_t)SYS_TIM_FREQUENCY))
#define TIME_MS2I(msecs) ((systime_t)((((uint64_t)(msecs) * (uint64_t)SYS_TIM_FREQUENCY) + (uint64_t)999)    / (uint64_t)1000))
#define TIME_US2I(usecs) ((systime_t)((((uint64_t)(usecs) * (uint64_t)SYS_TIM_FREQUENCY) + (uint64_t)999999) / (uint64_t)1000000))

#define TIME_I2S(interval)  (uint32_t)(( (uint64_t)(interval) + (uint64_t)SYS_TIM_FREQUENCY - (uint64_t)1) / (uint64_t)SYS_TIM_FREQUENCY)
...
public:
    ...
    inline void FastWaitI()   { cnt--; } // Can be used when the counter is known to be positive
    inline void FastSignalI() { cnt++; } // Can be used when the counter is known to be not negative
    void SignalI();
    void Signal();
